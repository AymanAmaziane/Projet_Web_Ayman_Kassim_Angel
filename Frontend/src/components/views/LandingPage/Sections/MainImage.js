...
function MainImage(props) {
    ...
    const Countdown = () => {  //Le compte à rebourd
        setInterval(() => {  //Pour avoir un compte à rebourd dynamique
            var date = new Date();
    ...
    const CheckCountdown = () => {  //si le film n'est toujours pas sortie on crée les cases pour placer le compte à rebourd 
        var date = new Date();
        var date1 = new Date(date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate() + " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds())
        var date2 = new Date(props.date + " 00:00:00");
        if (date1 < date2) {
